...
def _para_cm(valor: str, unidade: str | None) -> float | None:
    ...
    if unidade_norm in {"mm", "milimetro", "milimetros"}:
        return numero / 10.0
    if unidade_norm in {"m", "metro", "metros"}:
        return numero * 100.0
    if unidade_norm in {
        "pol",
        "polegada",
...
def _corrigir_escala_dimensional_suspeita(
    valores_convertidos: list[float],
    ...
    brutos_ordenados = sorted(
        valores_brutos,
        reverse=True,
    )

    unidades_norm = [
        normalizar_texto(unidade or "")
    ...
    escala_cm_plausivel = (
        4.0 <= brutos_ordenados[0] <= 30.0
        and brutos_ordenados[1] <= 15.0
    )

    if (
        tem_mm
        and maior_convertida < 4.0
        and escala_cm_plausivel
    ):
...
def _extrair_multiplicacoes(
    ...
    padrao = re.compile(
        r"(?P<a>\d{1,3}(?:[.,]\d+)?)\s*"
        r"(?P<ua>mm|cm|m|pol|polegadas?|in|inch(?:es)?)?"
        r"\s*(?:x|×|por)\s*"
        r"(?P<b>\d{1,3}(?:[.,]\d+)?)\s*"
        r"(?P<ub>mm|cm|m|pol|polegadas?|in|inch(?:es)?)?"
        r"(?:\s*(?:x|×|por)\s*"
        r"(?P<c>\d{1,3}(?:[.,]\d+)?)\s*"
        r"(?P<uc>mm|cm|m|pol|polegadas?|in|inch(?:es)?)?)?",
        flags=re.IGNORECASE,
    )
    ...
    for match in padrao.finditer(texto):
        contexto = texto[
            ...
            min(len(texto), match.end() + 100)
        ]
        contexto_norm = normalizar_texto(contexto)

        if not _contexto_valido(contexto):
            continue
        ...
        numeros_brutos = [
            item for item in numeros_brutos
            if item is not None
        ]
        ...
        unidade_padrao = (
            ...
            or match.group("ub")
            or match.group("ua")
            or _inferir_unidade_sem_rotulo(numeros_brutos)
        )
        ...
        valores, escala_corrigida = (
            _corrigir_escala_dimensional_suspeita(
                valores,
                numeros_brutos,
                unidades,
            )
        ...
        if any(valor <= 0 or valor > 80 for valor in valores):
            continue

        tem_rotulo = any(
            rotulo in contexto_norm
...
def _extrair_atributos_separados(
    ...
    for chave, valor in atributos.items():
        ...
        if not _contexto_valido(chave_norm):
            continue

        tipo = ""
        if "altura" in chave_norm or "comprimento" in chave_norm:
            tipo = "altura"
        elif "largura" in chave_norm:
            tipo = "largura"
        elif "espessura" in chave_norm or "profundidade" in chave_norm:
            tipo = "espessura"
        ...
        match = re.search(
            r"(\d{1,3}(?:[.,]\d+)?)\s*"
            r"(mm|cm|m|pol|polegadas?|in|inch(?:es)?)?",
            str(valor),
            flags=re.IGNORECASE,
        )
        if not match:
        ...
        brutos[tipo] = {
            ...
            "evidencia": f"{chave}: {valor}",
        }

    if "altura" not in brutos or "largura" not in brutos:
        return []

    numeros_sem_unidade = [
        item["numero"]
        for item in brutos.values()
        if not item["unidade"]
    ]

    unidade_inferida = (
        _inferir_unidade_sem_rotulo(
            numeros_sem_unidade
        )
        if numeros_sem_unidade
        else ""
    )
    ...
    if "altura" not in encontrados or "largura" not in encontrados:
        return []

    valores = [
        encontrados["altura"][0],
        encontrados["largura"][0],
    ]
    evidencias = [
        encontrados["altura"][1],
        encontrados["largura"][1],
    ]

    if "espessura" in encontrados:
        valores.append(
            encontrados["espessura"][0]
        )
        evidencias.append(
            encontrados["espessura"][1]
        )
...
def analisar_dimensoes_produto(
    dados: dict[str, Any],
) -> dict[str, Any]:
    ...
    for chave, valor in atributos.items():
        ...
        if not _contexto_valido(contexto):
            continue
        candidatos.extend(
            _extrair_multiplicacoes(
                contexto,
                origem=f"atributo:{chave}",
                prioridade=0,
            )
        )
    ...
    if not candidatos:
        return AnaliseDimensional().para_dict()
    ...
    dentro = (
        maior <= LIMITE_ALTURA_CM
        and segunda <= LIMITE_LARGURA_CM
    )
    ...
    return AnaliseDimensional(
        ...
        motivo_dimensoes=(
            "Dimensões corporais dentro do limite de 12 × 5,5 cm."
            if dentro
            else "Dimensões corporais acima do limite de 12 × 5,5 cm."
        ),
    ).para_dict()


def _analisar_indicios(
    dados: dict[str, Any],
) -> dict[str, str]:
    ...
    atributos_texto = normalizar_texto(
        " | ".join(f"{chave}: {valor}" for chave, valor in atributos.items())
    )
    texto_produto = " | ".join([
        titulo,
        descricao,
    ...
    evidencia_mini = (
        _primeiro_termo(texto_produto, TERMOS_MINI_FORTES)
        or _primeiro_termo(texto_produto, MODELOS_CONHECIDOS)
    )
    evidencia_telefonia = _primeiro_termo(
        texto_produto,
        TERMOS_TELEFONIA,
    )
    ...
    if evidencia_mini and not evidencia_telefonia:
        evidencia_telefonia = (
            "modelo/formato conhecido com histórico de telefonia"
        )
    ...
    return {
        ...
        "tem_indicios": (
            "SIM"
            if evidencia_mini or evidencia_telefonia
            else "NAO"
        ),
    }
...
def classificar_produto(
    dados: dict[str, Any],
    analise_dimensional: dict[str, Any],
    analise_anatel: dict[str, Any],
) -> dict[str, Any]:
    """Classifica somente depois das dimensões e da conferência Anatel."""

    indicios = _analisar_indicios(dados)
    dimensoes_confiaveis = (
        analise_dimensional.get("dimensoes_confiaveis") == "SIM"
    )
    dentro_limite = (
        analise_dimensional.get("dentro_limite_dimensional") == "SIM"
    )
    anatel_em_ordem = (
        analise_anatel.get("anatel_em_ordem") == "SIM"
    )
    codigo_anatel = str(
        analise_anatel.get("codigo_anatel_normalizado") or ""
    ).strip()
    ...
    if indicios["fora_escopo"] == "SIM":
        resultado.motivo_classificacao = (
            "Produto fora do escopo identificado pelo título: "
            f"{indicios['motivo_fora_escopo']}."
        )
    ...
    if not dimensoes_confiaveis:
        if indicios["tem_indicios"] == "SIM":
            ...
            if status_req in {"CANCELADA", "SUSPENSA"}:
                complemento_anatel = (
                    " Além disso, o código capturado aponta "
    ...
    if not dentro_limite:
        resultado.classificacao = "DESCARTADO"
        resultado.motivo_classificacao = (
            "Dimensões corporais maiores que o limite de 12 × 5,5 cm; "
            "produto descartado do escopo de mini celulares."
        )
        return resultado.para_dict()
    ...
    if indicios["tem_indicios"] != "SIM":
        resultado.classificacao = "DESCARTADO"
        resultado.motivo_classificacao = (
            "Dimensões reduzidas encontradas, mas o anúncio não apresenta "
            "indícios suficientes de telefone celular."
        )
        return resultado.para_dict()

    status_requerimento = analise_anatel.get(
        "situacao_requerimento_normalizada"
    )
    if status_requerimento in {"CANCELADA", "SUSPENSA"}:
        resultado.classificacao = "IRREGULAR"
        situacao_original = (
            analise_anatel.get("situacao_requerimento_base")
            or f"Homologação {status_requerimento.lower()}"
        )
        resultado.motivo_classificacao = (
            "Dimensões dentro do limite de 12 × 5,5 cm e o código "
            f"capturado corresponde a '{situacao_original}' na base Anatel. "
            "Homologação cancelada ou suspensa torna o produto irregular."
        )
        return resultado.para_dict()
