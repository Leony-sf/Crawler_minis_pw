...
# Função embutida para tornar o arquivo 100% independente do ML
...
TERMOS_TELEFONIA = {
    "telefone celular", "celular", "mini celular", "mini phone", "micro celular",
    "phone", "dual sim", "dois chips", "2 chips", "sim card", "cartao sim",
    "chip gsm", "gsm", "imei", "sms", "faz chamadas", "realiza chamadas",
    "chamada de voz", "rede 2g", "rede 3g", "rede 4g", "lte", "bluetooth dialer",
    "discador bluetooth",
}

TERMOS_MINI_FORTES = {
    "mini celular", "mini phone", "micro celular", "smallest phone", "tiny phone",
    "telefone gsm pequeno", "telefone cartao", "card phone", "ponto eletronico celular",
    "celular caneta", "celular batom", "celular isqueiro", "celular chaveiro",
    "celular chave de carro",
}

MODELOS_CONHECIDOS = {
    "bm10", "bm20", "bm30", "bm50", "bm70", "bm90", "bm100", "bm200", "bm310",
    "bt11", "bt22", "b25", "b30", "b68", "cat b68", "j8", "j9", "j10", "k10",
    "k33", "k66", "soyes", "melrose", "long cz", "zanco", "l8star", "anica",
    "i17 pro", "i17 pro max",
}
...
ROTULOS_DIMENSAO = {
    ...
    "medida do produto", "altura", "largura", "comprimento", "profundidade", "espessura",
}

ROTULOS_EXCLUIR = {
    "embalagem", "pacote", "caixa", "display", "tela", "screen", "diagonal",
    "monitor", "volume", "frete", "produto embalado",
}
...
def _primeiro_termo(texto: str, termos: set[str]) -> str:
    for termo in sorted(termos, key=len, reverse=True):
        if termo in texto: return termo
    return ""
...
def _corrigir_escala_dimensional_suspeita(valores_convertidos: list[float], valores_brutos: list[float], unidades: list[str | None]) -> tuple[list[float], bool]:
    ...
    unidades_norm = [normalizar_texto(u or "") for u in unidades if u]
    tem_mm = "mm" in unidades_norm
    escala_cm_plausivel = (4.0 <= brutos_ordenados[0] <= 30.0 and brutos_ordenados[1] <= 15.0)
    if tem_mm and maior_convertida < 4.0 and escala_cm_plausivel:
        return list(valores_brutos), True
    return valores_convertidos, False

def _extrair_multiplicacoes(texto: str, origem: str, prioridade: int) -> list[dict[str, Any]]:
    padrao = re.compile(
        r"(?P<a>\d{1,3}(?:[.,]\d+)?)\s*(?P<ua>mm|cm|m|pol|polegadas?|in|inch(?:es)?)?\s*(?:x|×|por)\s*"
        r"(?P<b>\d{1,3}(?:[.,]\d+)?)\s*(?P<ub>mm|cm|m|pol|polegadas?|in|inch(?:es)?)?"
        r"(?:\s*(?:x|×|por)\s*(?P<c>\d{1,3}(?:[.,]\d+)?)\s*(?P<uc>mm|cm|m|pol|polegadas?|in|inch(?:es)?)?)?",
        flags=re.IGNORECASE,
    )
    ...
    for match in padrao.finditer(texto):
        ...
        numeros_brutos = [item for item in [_numero(b) for b in brutos] if item is not None]
        ...
        candidatos.append({
            "valores": ordenados,
            "evidencia": " ".join(contexto.split()),
            "origem": (origem + " [escala reavaliada]" if escala_corrigida else origem),
            "prioridade": prioridade - (1 if tem_rotulo else 0),
        })
    return candidatos

def _extrair_atributos_separados(atributos: dict[str, str]) -> list[dict[str, Any]]:
    brutos: dict[str, dict[str, Any]] = {}
    for chave, valor in atributos.items():
        chave_norm = normalizar_texto(chave)
        if not _contexto_valido(chave_norm): continue
        tipo = ""
        if "altura" in chave_norm or "comprimento" in chave_norm: tipo = "altura"
        elif "largura" in chave_norm: tipo = "largura"
        elif "espessura" in chave_norm or "profundidade" in chave_norm: tipo = "espessura"
    ...
    encontrados: dict[str, tuple[float, str]] = {}
    for tipo, item in brutos.items():
        unidade = item["unidade"] or unidade_inferida or "cm"
        convertido = _para_cm(item["numero_texto"], unidade)
...
def analisar_dimensoes_produto(dados: dict[str, Any]) -> dict[str, Any]:
    ...
    return AnaliseDimensional(
        ...
        maior_dimensao_cm=maior,
        segunda_dimensao_cm=segunda,
        evidencia_dimensoes=escolhido["evidencia"],
        origem_dimensoes=escolhido["origem"],
        motivo_dimensoes="Dimensões corporais dentro do limite de 12 × 5,5 cm." if dentro else "Dimensões corporais acima do limite de 12 × 5,5 cm.",
    ).para_dict()
...
def _analisar_indicios(dados: dict[str, Any]) -> dict[str, str]:
    ...
    if evidencia_mini and not evidencia_telefonia: evidencia_telefonia = "modelo/formato conhecido com histórico de telefonia"
    return {
...
def classificar_produto(dados: dict[str, Any], analise_dimensional: dict[str, Any], analise_anatel: dict[str, Any]) -> dict[str, Any]:
    """Classifica com PRIORIDADE para o limite de dimensões e validação Anatel."""
    ...
    # Extrai o status cravado pela base Anatel
    status_anatel = analise_anatel.get("situacao_anatel", "NAO_VERIFICADO")
    ...
    # 1. Prioridade Máxima Absoluta: Fora de Escopo (Acessórios, peças, etc)
    ...
    # 2. NOVA PRIORIDADE 1: Acima do Limite Dimensional
    # Se o robô encontrou uma medida e ela ultrapassa o limite de um mini celular, DESCARTA na hora!
    if dimensoes_confiaveis and not dentro_limite:
        resultado.classificacao = "DESCARTADO"
        resultado.motivo_classificacao = "Dimensões corporais maiores que o limite de 12 × 5,5 cm; produto descartado do escopo de mini celulares."
        return resultado.para_dict()

    # 3. Prioridade 2: ANATEL IRREGULAR (Código falso, cancelado, suspenso ou sem registro)
    if status_anatel == "IRREGULAR":
        resultado.classificacao = "IRREGULAR"
        resultado.motivo_classificacao = motivo_anatel
        return resultado.para_dict()

    # 4. Prioridade 3: ANATEL REVISAR (Divergência de marca/modelo/nome comercial)
    if status_anatel == "REVISAR":
        resultado.classificacao = "NÃO CLASSIFICADO"
        resultado.motivo_classificacao = motivo_anatel
        return resultado.para_dict()

    # 5. Prioridade 4: ANATEL REGULAR (Tudo certo)
    if status_anatel == "REGULAR":
        resultado.classificacao = "DESCARTADO"
        resultado.motivo_classificacao = "Produto regular: código Anatel, marca, modelo técnico e nome comercial conferem com a base."
        return resultado.para_dict()
    ...
    if indicios["tem_indicios"] != "SIM":
        resultado.classificacao = "DESCARTADO"
        resultado.motivo_classificacao = "Dimensões reduzidas encontradas, mas o anúncio não apresenta indícios suficientes de telefone celular."
        return resultado.para_dict()

    # Se chegou no fim, é um mini celular comprovado (medidas ok + indícios) MAS sem selo da Anatel
    resultado.classificacao = "IRREGULAR"
    resultado.motivo_classificacao = "Dimensões dentro do limite de 12 × 5,5 cm e código Anatel não localizado no anúncio."

    return resultado.para_dict()

    # Se chegou no fim, é um mini celular (pequeno + telefonia) MAS sem selo da Anatel
    resultado.classificacao = "IRREGULAR"
    resultado.motivo_classificacao = "Dimensões dentro do limite de 12 × 5,5 cm e código Anatel não localizado no anúncio."
