...
def _texto_compativel(anuncio: str, base: str) -> bool:
    anuncio_norm, base_norm = normalizar_texto(anuncio), normalizar_texto(base)
...
def carregar_base_anatel(caminho: str | Path | None) -> BaseAnatel | None:
    ...
    coluna_modelo = _achar_coluna(df, [["modelo"], ["nome", "modelo"]])
    ...
    return BaseAnatel(dataframe=base, coluna_codigo=coluna_codigo, coluna_fabricante=coluna_fabricante, coluna_modelo=coluna_modelo, coluna_situacao_requerimento=coluna_situacao_requerimento)
...
def analisar_situacao_anatel(codigo: str, marca: str, modelo: str, base: BaseAnatel | None) -> dict[str, str]:
    ...
    resultado = {
        ...
        "codigo_confere_base": "NAO", "marca_confere_base": "NAO", "modelo_confere_base": "NAO",
    ...
    if encontrados.empty:
        resultado.update({"situacao_anatel": "IRREGULAR", "motivo_anatel": "Código sem correspondência na base."})
        return resultado
    ...
    resultado.update({
        ...
        "fabricante_base": fabricante_base, "modelo_base": modelo_base, "situacao_requerimento_base": situacao_requerimento_base,
        "situacao_requerimento_normalizada": situacao_requerimento_normalizada,
        "marca_confere_base": "SIM" if marca_confere else "NAO", "modelo_confere_base": "SIM" if modelo_confere else "NAO",
    ...
    elif not marca_confere: divergencias.append("marca diferente da base")

    if not modelo: divergencias.append("modelo não capturado")
    elif not modelo_base: divergencias.append("modelo ausente na base")
    elif not modelo_confere: divergencias.append("modelo diferente da base")
    ...
    if situacao_requerimento_normalizada != "EMITIDA":
        resultado.update({"situacao_anatel": "REVISAR", "motivo_anatel": f"Valor não reconhecido: {situacao_requerimento_base}."})
        return resultado
    if divergencias:
        resultado.update({"situacao_anatel": "REVISAR", "motivo_anatel": "Homologação Emitida, porém " + "; ".join(divergencias) + "."})
        return resultado
