...
def _log_auditoria_anatel(dados: DadosProduto, anatel: dict[str, Any], modelo_anuncio: str) -> None:
    ...
    log("anatel", "Nome Comercial : anúncio=" + _valor_terminal(dados.nome_comercial) + " | base=" + _valor_terminal(anatel.get("nome_comercial_base")) + " | confere=" + _sim_nao_terminal(anatel.get("nome_comercial_confere_base")))
...
async def _salvar_print(page: Page, pasta_saida: Path, linha: dict[str, Any]) -> str:
    ...
    if classificacao not in {"IRREGULAR", "SUSPEITO", "NAO_CLASSIFICADO"}: return ""
    
    if classificacao == "IRREGULAR": pasta = pasta_saida / "prints" / "irregulares"
    elif classificacao == "SUSPEITO": pasta = pasta_saida / "prints" / "suspeitos"
    else: pasta = pasta_saida / "prints" / "nao_classificados"
...
async def _criar_contexto_persistente(p: Any) -> BrowserContext:
    ...
    return await p.chromium.launch_persistent_context(
        user_data_dir=str(perfil), headless=False, viewport={"width": 1366, "height": 900},
        locale="pt-BR", args=["--disable-blink-features=AutomationControlled", "--start-maximized"]
    )

async def rodar_playwright_carrefour(
    query: str, queries: list[str], limite: int, base_anatel: BaseAnatel | None,
    url: str | None, saida: str | Path | None, max_paginas: int, pausar_inicio: bool, porta_chrome: int
) -> dict[str, Any]:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for indice_busca, consulta_atual in enumerate(buscas, start=1):
                ...
                if not url:
                    await page.goto(f"https://www.carrefour.com.br/busca/{quote(consulta_atual)}", wait_until="domcontentloaded", timeout=60000)
                ...
                while True:
                    ...
                    for href in links_novos:
                        ...
                        try:
                            ...
                            anatel = analisar_situacao_anatel(dados.codigo_anatel_principal, dados.marca, modelo_anatel, dados.nome_comercial, base_anatel)
                            ...
                            linha = {
                                ...
                                "marketplace_id": "3", "marketplace": "carrefour",
                                "titulo": dados.titulo, "link": dados.url,
                                ...
                                "marca": dados.marca, "preco": dados.preco,
                                "modelo": dados.modelo, "modelo_decisivo": modelo_anatel, "nome_comercial": dados.nome_comercial,
                                "classificacao": classificacao_final, "status_validacao": classificacao_final,
                                ...
                                "warning": classificacao.get("motivo_classificacao", "") if classificacao_final in {"SUSPEITO", "NAO_CLASSIFICADO"} else anatel.get("motivo_anatel", ""),
                        ...
                        except Exception as exc: log("erro", f"Falha ao processar anúncio: {exc}")
                        finally: await prod_page.close()
                    pagina_atual += 1
                    break
                if url: break
        finally: log("chrome", "Execução finalizada.")

    return {"pasta_saida": str(pasta_saida.resolve()), "total_visitados": total_processados, "total_descartados": total_descartados, "total_produtos_no_parquet": len(linhas)}